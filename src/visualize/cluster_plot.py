...
def cluster_name(f1, f2, f3, f4):
    ...
    if f3 > 0.5:
        return "典型环境噪声暴露型"
    if max(f1, f2, f3, f4) < 0:
        return "低敏感低困扰型"
    return "一般型"


def build_name_map(profiles):
    return {
        int(r["cluster"]): cluster_name(r["F1"], r["F2"], r["F3"], r["F4"])
        for _, r in profiles.iterrows()
    }


def plot_radar(profiles, name_map, out):
    ...
    for _, row in profiles.iterrows():
        ...
        ax.plot(angles, vals, color=c, linewidth=2, label=name_map[int(row["cluster"])])
...
def plot_3d(result, name_map, out):
    ...
    for c in sorted(result["cluster"].unique()):
        m = result["cluster"] == c
        ax.scatter(z[m, 0], z[m, 1], z[m, 2], s=18, color=CLUSTER_COLORS[c], label=name_map[c])
...
def main():
    ...
    name_map = build_name_map(profiles)
    plot_radar(profiles, name_map, os.path.join(FIG_DIR, "cluster_radar.png"))
    plot_3d(result, name_map, os.path.join(FIG_DIR, "cluster_3d.png"))
